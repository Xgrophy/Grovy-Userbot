...
@grovy_cmd(pattern="promote(?:\\s|$)([\\s\\S]*)")
...
async def promote(event):
    ...
    eventgrovy = await edit_or_reply(event, "`Promosikan Pengguna Sebagai Admin... Mohon Menunggu`")
    ...
    try:
        ...
        await edit_delete(eventgrovy, "`Berhasil Menambahkan Pengguna Ini Sebagai Admin!`")
...
@grovy_cmd(pattern="demote(?:\\s|$)([\\s\\S]*)")
...
async def demote(event):
    ...
    eventgrovy = await edit_or_reply(event, "`Sedang Melepas Admin...`")
...
async def ban(bon):
    ...
    grovy = await edit_or_reply(bon, "`Melakukan Banned!`")
...
async def nothanos(unbon):
    ...
    grovy = await edit_or_reply(unbon, "`Sedang Melakukan Unban...`")
    ...
    try:
        ...
        await edit_delete(grovy, "```Berhasil Melepas Ban Pengguna!```")
        ...
        await edit_delete(grovy, "`Sepertinya Terjadi Kesalahan!`")


@grovy_cmd(pattern="mute(?: |$)(.*)")
...
async def spider(spdr):
    ...
    grovy = await edit_or_reply(spdr, "`Sedang melakukan Mute...`")
    ...
    await grovy.edit("`Telah Dibisukan!`")
    if mute(spdr.chat_id, user.id) is False:
        return await grovy.edit("`Pengguna Sudah Dibisukan!`")
    else:
        try:
            ...
            if reason:
                await grovy.edit(f"**Pengguna Telah Dibisukan!**\n**Alasan:** `{reason}`")
            else:
                await grovy.edit("`Telah Dibisukan!`")
...
async def unmoot(unmot):
    ...
    grovy = await edit_or_reply(unmot, "```Melakukan Unmute...```")
    ...
    if unmute(unmot.chat_id, user.id) is False:
        ...
        try:
            ...
            await edit_delete(grovy, "```Berhasil Melakukan Unmute! Pengguna Sudah Tidak Dibisukan```")
        except UserIdInvalidError:
            return await edit_delete(grovy, "`Terjadi Kesalahan!`")
...
async def ungmoot(un_gmute):
    ...
    grovy = await edit_or_reply(un_gmute, "`Sedang melakukan membuka Global Mute...`")
    ...
    await grovy.edit("```Membuka Global Mute Pengguna...```")

    if ungmute(user.id) is False:
        await grovy.edit("`Pengguna Sedang Tidak Di Gmute!`")
    else:
...
async def gspider(gspdr):
    ...
    grovy = await edit_or_reply(gspdr, "`Processing...`")
    user, reason = await get_user_from_event(gspdr)
    if not user:
        return

    # If pass, inform and start gmuting
    await grovy.edit("`Pengguna Berhasil Dibisukan!`")
    if gmute(user.id) is False:
        await edit_delete(gspdr, "`Kesalahan! Pengguna Sudah Dibisukan.`")
    else:
        if reason:
            await grovy.edit(f"**Dibisukan Secara Global!**\n**Alasan:** `{reason}`")
        else:

...
from userbot.utils import edit_or_reply, edit_delete, grovy_cmd
from userbot import BOTLOG, BOTLOG_CHATID, CMD_HELP, CMD_HANDLER as cmd


@grovy_cmd(pattern="sspam(?: |$)(.*)")
async def stickerpack_spam(event):
    ...
    try:
        ...
        grovy = await edit_or_reply(event, "`Fetching details of the sticker pack, please wait..`"
                                    )
    except BaseException:
        await grovy.edit("`This is not a sticker. Reply to a sticker.`", 5)
        return
    try:
        ...
        return await grovy.edit("`I guess this sticker is not part of any pack so i cant kang this sticker pack try kang for this sticker`",
                                )
    try:
        hmm = Get(hmm)
